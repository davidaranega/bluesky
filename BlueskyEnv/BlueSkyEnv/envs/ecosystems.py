...
class EcosystemAreas:
	...
	def __init__(self, lat, lon, v, alpha_max, alt, time_horizon = 600, time_steps = 10):
		self.v = v
		self.lat_o = lat
		self.lon_o = lon
		self.alt_o = alt
...
def calculate_tlosh(area1, area2, dlosh):
	for index in range(len(area1.xecef)):
		if index == range(len(area1.xecef)):
		...
		for index2 in range(len(area2.xecef)):
			...
			if index2 == range(len(area2.xecef)):
				e2 = np.array([area2.xecef[0], area2.yecef[0], area2.zecef[0]]) - np.array([area2.xecef[index], area2.yecef[index], area2.zecef[index]])
			else:
				e2 = np.array([area2.xecef[index+1], area2.yecef[index+1], area2.zecef[index+1]]) - np.array([area2.xecef[index], area2.yecef[index], area2.zecef[index]])
